...
export default class Main extends Controller {
    ...
    public onAfterRendering(): void {
        ...
        view = this.getView(),
        mFBConditions: JSONModel = new JSONModel({
            filtersTextInfo: filterBar.getActiveFiltersText() as object
        });
        if (view) {
    ...
    public onFiltersChanged(event: UI5Event) : void {
        ...
        mFBConditions: JSONModel = filterBar.getModel("fbConditions") as JSONModel;
    ...
    public onPressed(event: UI5Event) : void {
        ...
        if (oContext) {
            const routing = this.getExtensionAPI().getRouting() as Routing;
